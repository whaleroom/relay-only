...
export { WHL_CONTRACT, WHLC_CONTRACT, V2_ROUTER, V2_FACTORY, WETH, rpcRead, quoteExactOutput, getV2Pair, getV2Reserves, getV2Token0, calcAmountIn, getEthUsdPrice, sendTx, waitForReceipt, getWallet, encodeCall, stakeWHL, approveWHL, readWHLBalance }
...
function StakeCard ({ wallet, balances, onRefresh }) {
  ...
  async function doApprove () {
    ...
    try {
      ...
      showToast('WHL approved for staking — no future approvals needed', 'success')
    } catch (err) {
  ...
  async function doStake () {
    ...
    try {
      ...
      if (needsApproval && canBatch) {
        ...
        setNeedsApproval(false)
        showToast(`Committed ${amount} WHL (1-tx batch)`, 'success')
      } else {
        const hash = await stakeWHL(wei, ref)
        setTxHash(hash)
        await waitForReceipt(hash)
        showToast(`Committed ${amount} WHL`, 'success')
      }
      setAmount('')
      setCalc(null)
      onRefresh()
...
function UnstakeCard ({ wallet, balances, onRefresh }) {
  const [amount, setAmount] = useState('')
  const [pending, setPending] = useState(false)
  const [calc, setCalc] = useState(null)
  const [txHash, setTxHash] = useState('')
  ...
  async function doUnstake () {
    ...
    try {
      ...
      setTxHash(hash)
      await waitForReceipt(hash)
      showToast(`Returned ${amount} WHLC`, 'success')
      setAmount('')
      setCalc(null)
      setShowWarning(false)
      onRefresh()
...
function RewardsPanel ({ balances, onRefresh }) {
  ...
  async function doClaim () {
    ...
    try {
      ...
      await waitForReceipt(hash)
      showToast('Distributions claimed', 'success')
      onRefresh()
    } catch (err) {
  ...
  async function doCompound () {
    ...
    try {
      ...
      await waitForReceipt(hash)
      showToast('Distributions restaked', 'success')
      onRefresh()
    } catch (err) {
      showToast(err.message || 'Restake failed', 'error')
    } finally {
      setPending('')
    }
  }
...
function ContributeCard ({ onRefresh }) {
  const [amount, setAmount] = useState('')
  const [pending, setPending] = useState(false)
  const [needsApproval, setNeedsApproval] = useState(false)
  const [txHash, setTxHash] = useState('')
  ...
  async function doApprove () {
    ...
    try {
      ...
      await waitForReceipt(hash)
      setNeedsApproval(false)
      showToast('WHL approved — no future approvals needed', 'success')
    } catch (err) {
      showToast(err.message || 'Approval failed', 'error')
    } finally {
      setPending(false)
  ...
  async function doContribute () {
    ...
    try {
      ...
      if (needsApproval && canBatch) {
        ...
        await waitForBatch(batchId)
        setNeedsApproval(false)
        showToast(`Contributed ${amount} WHL (1-tx batch)`, 'success')
      } else {
        const hash = await contributeWHL(wei)
        setTxHash(hash)
        await waitForReceipt(hash)
        showToast(`Contributed ${amount} WHL to all members`, 'success')
      }
      setAmount('')
      onRefresh()
    } catch (err) {
      showToast(err.message || 'Contribute failed', 'error')
    } finally {
      setPending(false)
    }
  }
...
function FullExitCard ({ onRefresh }) {
  const [pending, setPending] = useState(false)
  ...
  async function doExit () {
    ...
    try {
      ...
      await waitForReceipt(hash)
      showToast('Full exit complete', 'success')
      onRefresh()
    } catch (err) {
...
function SwapCard ({ wallet, onRefresh }) {
  ...
  async function doSwap () {
    ...
    try {
      ...
      setTxHash(hash)
      await waitForReceipt(hash)
      showToast(`Swapped for ${totalWHL} WHL`, 'success')
      onRefresh()
    } catch (err) {
...
// ── Club View (inline center-column replacement) ──

export function ClubView ({ onBack, initialTab }) {
  ...
  return html`
    <div class="club-view">
      <div class="club-view-header">
        <button class="club-back-btn" onClick=${onBack}>
          ...
          <div>
            ...
            ${tab === 'overview' && html`
              <div class="club-tab-content">
                ...
                <${RewardsPanel} balances=${balances} onRefresh=${refresh} />
            ...
            ${tab === 'stake' && html`
              <div class="club-tab-content">
                <${StakeCard} wallet=${wallet} balances=${balances} onRefresh=${refresh} />
                <${ContributeCard} onRefresh=${refresh} />
            ...
            ${tab === 'getwhl' && html`
              <div class="club-tab-content">
                <${SwapCard} wallet=${wallet} onRefresh=${refresh} />
                <div class="club-card">
            ...
            ${tab === 'unstake' && html`
              <div class="club-tab-content">
                <${UnstakeCard} wallet=${wallet} balances=${balances} onRefresh=${refresh} />
                <div class="club-section-title">Danger Zone</div>
                <${FullExitCard} onRefresh=${refresh} />
              </div>
            `}
